			...
			<div className="container-enterprise">
				<div className="flex flex-col mb-24">
					<div className="flex items-center gap-4 mb-6">
						...
						<span className="text-primary-blue font-mono text-sm tracking-[0.2em] uppercase">{t('Open Requisitions')}</span>
					</div>
					<h2 className="text-white font-heading font-black text-5xl md:text-7xl uppercase tracking-tighter max-w-4xl leading-[0.9]">
						{t('STEP INTO')} <br />
						<span className="text-white/30">{t('THE ARENA.')}</span>
					</h2>
				</div>
								...
								<div className="flex flex-col md:flex-row md:items-center gap-6 md:gap-16 w-full">
									...
									<div className="flex flex-col gap-2 w-full md:w-1/2">
										...
										<div className="flex items-center gap-4 mt-2 opacity-60 group-hover:opacity-100 transition-opacity duration-300">
											<span className="font-mono text-xs uppercase tracking-widest text-white/80">{t(role.department)}</span>
										</div>
									</div>
									...
									<div className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-12 w-full md:w-auto mt-6 md:mt-0 opacity-60 group-hover:opacity-100 transition-opacity duration-300">
										<div className="flex flex-col gap-1">
											<span className="text-white/40 font-mono text-[10px] uppercase tracking-widest">{t('Location')}</span>
											<span className="text-white text-sm font-medium tracking-wide">{t(role.location)}</span>
										</div>
										<div className="flex flex-col gap-1">
											<span className="text-white/40 font-mono text-[10px] uppercase tracking-widest">{t('Type')}</span>
											<span className="text-white text-sm font-medium tracking-wide">{t(role.type)}</span>
										</div>
