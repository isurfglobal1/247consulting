...
export function ServicesIndustries() {
	...
	const industries = [
		...
		{
			name: t('Technology'),
			img: 'https://images.unsplash.com/photo-1528901166007-3784c7dd3653?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxhZnJpY2FuJTIwdGVjaG5vbG9neSUyMGRldmVsb3BlcnxlbnwxfHx8fDE3ODAwMTE5MTF8MA&ixlib=rb-4.1.0&q=80&w=1080',
		},
		{
			name: t('Retail & Logistics'),
			img: 'https://images.unsplash.com/photo-1587293852726-70cdb56c2866?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxtb2Rlcm4lMjByZXRhaWwlMjBsb2dpc3RpY3N8ZW58MXx8fHwxNzgwMDExOTA2fDA&ixlib=rb-4.1.0&q=80&w=1080',
		},
		{
	...
	const additionalIndustries = [t('Telecom'), t('Healthcare'), t('Government'), t('Oil & Gas')];
			...
			<div className="container-enterprise relative z-10">
				<motion.div
					...
					<h2 className="text-white text-4xl md:text-5xl lg:text-7xl font-heading font-black tracking-tight leading-[1.05]">
						{t('Operational Sectors')} <br /> <span className="text-primary-blue">{t('Dominated')}</span>
					</h2>
					<p className="text-white/50 text-lg max-w-sm md:text-right">{t('Cross-industry expertise powering the continents most demanding corporate sectors.')}</p>
				</motion.div>
						...
						<motion.div
							...
							<div className="absolute bottom-0 left-0 p-8 w-full translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
								<div className="flex items-center gap-4 mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
									...
									<span className="text-white text-xs font-bold tracking-widest uppercase">{t('Active Deployment')}</span>
								</div>
								<h3 className="text-white text-2xl lg:text-3xl font-heading font-bold">{ind.name}</h3>
							</div>
						</motion.div>
					))}
				</div>
