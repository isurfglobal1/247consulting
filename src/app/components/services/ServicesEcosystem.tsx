      ...
      <div className="container-enterprise relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-24 gap-8">
          ...
          <p className="text-white/50 text-lg max-w-sm">
            Interact with the index below to explore our core enterprise deployments.
          </p>
        </div>
            ...
            {services.map((svc, idx) => {
              ...
              return (
                <div 
                  ...
                  className={`group relative border-t ${idx === services.length - 1 ? 'border-b' : ''} border-white/10 py-10 cursor-pointer transition-colors duration-500 hover:bg-white/[0.02]`}
                >
                  ...
                  <div className="flex items-start gap-8 px-4 lg:px-8">
                    <span className={`font-mono text-sm mt-2 transition-colors duration-500 ${isActive ? 'text-primary-blue' : 'text-white/30'}`}>
                      {svc.num}
                    </span>
                    
                    <div className="flex-1">
                      <h3 className={`text-3xl md:text-5xl font-heading font-black tracking-tight transition-colors duration-500 ${isActive ? 'text-white' : 'text-white/40 group-hover:text-white/70'}`}>
                        {svc.title}
                      </h3>
                      ...
                      <AnimatePresence>
                        {isActive && (
                          ...
                          >
                            ...
                            <Link to={`/services/${svc.id}`} className="inline-flex items-center gap-3 text-primary-blue font-bold text-sm uppercase tracking-widest hover:text-white transition-colors">
                              {svc.cta}
                              <ArrowRight size={16} />
                            </Link>
                          </motion.div>
            ...
            <div className="w-full h-full relative rounded-none overflow-hidden bg-[#111]">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="absolute bottom-6 right-6 text-white/30 font-mono text-xs">
                    {services[activeService].id.toUpperCase()}_SYS
                  </div>
                </motion.div>
              </AnimatePresence>
            </div>
