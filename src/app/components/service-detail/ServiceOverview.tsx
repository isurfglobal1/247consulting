				...
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 items-center">
					...
					<motion.div
						...
						<div className="absolute inset-0 rounded-2xl overflow-hidden border border-white/10 bg-[#111]">
							<ImageWithFallback
								src={
									serviceData?.narrative?.image ||
									'https://images.unsplash.com/photo-1573164713712-03790a178651?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxhZnJpY2FuJTIwcHJvZmVzc2lvbmFsJTIwc2VydmVyfGVufDF8fHx8MTc4MDAxMjkxMnww&ixlib=rb-4.1.0&q=80&w=1080'
								}
								alt={t('serviceOverview.imageAlt', 'African Professional Server Operations')}
						...
						<motion.div
							...
							<div className="text-primary-blue text-xs font-mono uppercase tracking-widest mb-4 flex justify-between">
								<span>{t('serviceOverview.routingStatus', 'Routing Status')}</span>
								<span>[{t('serviceOverview.optimal', 'Optimal')}]</span>
							</div>
							<div className="space-y-3">
								<div className="flex justify-between text-white/70 text-sm">
									<span>{t('serviceOverview.matchRate', 'Candidate Match Rate')}</span>
					...
					<motion.div
						...
						<div className="text-white/40 text-xs font-bold tracking-widest uppercase mb-6 flex items-center gap-4">
							...
							{t('serviceOverview.narrativeLabel', 'Operational Narrative')}
						</div>

						<h2 className="text-white text-4xl md:text-5xl lg:text-6xl font-heading font-black tracking-tight leading-[1.1] mb-8">
							{t('serviceOverview.title1', 'Replacing Guesswork')} <br /> {t('serviceOverview.title2', 'With')}{' '}
							<span className="text-white/30">{t('serviceOverview.title3', 'Architecture.')}</span>
						</h2>

						<div className="space-y-6 text-white/60 text-lg font-light leading-relaxed">
							<p>
								{t(
									`services.${serviceId}.narrative.para1`,
									t(
							...
							<p>
								{t(
									`services.${serviceId}.narrative.para2`,
									t(
						...
						<div className="mt-12 grid grid-cols-2 gap-8 pt-8 border-t border-white/10">
							<div>
								...
								<div className="text-xs text-white/40 uppercase tracking-widest font-mono">{t('serviceOverview.reliability', 'System Reliability')}</div>
							</div>
							<div>
								<div className="text-3xl font-heading font-black text-white mb-2">{t('serviceOverview.multiGeo', 'Multi-Geo')}</div>
								<div className="text-xs text-primary-blue uppercase tracking-widest font-mono">{t('serviceOverview.deploymentReady', 'Deployment Ready')}</div>
							</div>
						</div>
					</motion.div>
				</div>
